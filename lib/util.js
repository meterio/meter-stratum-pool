...
exports.bufferToCompactBits = function (startingBuff) {
  var buff = startingBuff;

  buff =
    buff.readUInt8(0) > 0x7f
      ? Buffer.concat([Buffer.from("0x00", "hex"), buff])
      : buff;

  buff = Buffer.concat([Buffer.from([buff.length]), buff]);
  var compact = buff.subarray(0, 4);
  return compact;
};
...
exports.convertToBE = function (s) {
  ...
  result = data.reverse().join("").toString(16);
