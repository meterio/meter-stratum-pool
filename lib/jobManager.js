...
var JobManager = (module.exports = function JobManager(options) {
  ...
  this.updateCurrentJob = function (rpcData) {
    const beneficiary = options.rewardBeneficiary
      .toLowerCase()
      .replace("0x", "");
  ...
  this.processShare = function (
    ...
    var headerHashBE = util.convertToBE(headerHash);
    var headerBigNum = new BigNumber("0x" + headerHashBE.toString("hex"));
    ...
    if (new BigNumber(job.target.toString()).gte(headerBigNum)) {
      ...
      if (
        options.coin.algorithm === "blake" ||
        options.coin.algorithm === "neoscrypt"
      ) {
        blockHash = util
          .reverseBuffer(util.sha256d(headerBuffer, nTime))
          .toString("hex");
      } else {
